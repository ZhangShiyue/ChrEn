...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
                 ...
                 alignment_heads=0, bert_attn=False):
        ...
        if self.bert_attn:
            self.input_map = nn.Linear(768, d_model)
            self.output_map = nn.Linear(2 * d_model, d_model)
            self.attn_bert = MultiHeadedAttention(heads, d_model, dropout=attention_dropout)
    ...
    def _forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
        ...
        if self.bert_attn:
            memory_bank, bert_memory_bank = memory_bank[:, :, :self.hidden_size], \
                                            memory_bank[:, :, self.hidden_size:]
            bert_memory_bank = self.input_map(bert_memory_bank)
        ...
        mid, attns = self.context_attn(memory_bank, memory_bank, query_norm,
                                       ...
                                       attn_type="context")
        if self.bert_attn:
            bert_mid, _ = self.attn_bert(bert_memory_bank, bert_memory_bank, query_norm,
                                         mask=src_pad_mask, layer_cache=layer_cache,
                                         attn_type="context")
            mid = torch.cat([mid, bert_mid], dim=2)
            mid = self.output_map(mid)
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayer(d_model, heads, d_ff, dropout,
             ...
             alignment_heads=alignment_heads, bert_attn=bert_attn)
