...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        if self.bert_attn:
            self.input_map = nn.Linear(768, hidden_size)
            self.output_map = nn.Linear(2 * hidden_size, hidden_size)
            self.attn_bert = GlobalAttention(
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None):
        ...
        for emb_t in emb.split(1):
            ...
            if self.attentional:
                ...
                if self.bert_attn:
                    decoder_output_bert, _ = self.attn_bert(rnn_output, bert_memory_bank.transpose(0, 1),
                                                            memory_lengths=memory_lengths)
                    decoder_output = torch.cat([decoder_output, decoder_output_bert], dim=1)
                    decoder_output = self.output_map(decoder_output)
            else:
