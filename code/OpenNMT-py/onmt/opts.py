...
def model_opts(parser):
    ...
    group.add('--bert_model', '-bert_model', type=str, default='bert-base-cased',
              choices=['bert-base-cased', 'bert-base-uncased', 'bert-base-multilingual-cased'],
              help="Which bert model to use")
    group.add('--bert_layers', '-bert_layers', type=int, default=0,
              help="How many bert layers to use")
    group.add('--bert_input', '-bert_input', action="store_true",
              help="Use bert's output as encoder's input")
    group.add('--bert_output', '-bert_output', action="store_true",
              help="Concatenate bert's output to encoder's output")
    group.add('--bert_attn', '-bert_attn', action="store_true",
              help="Use another decoder attention for bert's output")
    group.add('--cache_dir', '-cache_dir', type=str, default='None',
              help="Cache dir to save pretrained bert model")
...
def train_opts(parser):
    ...
    group.add('--save_checkpoint_steps', '-save_checkpoint_steps',
              ...
              only save the best and the last checkpoints by setting this to 0""")
